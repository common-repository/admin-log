...
Plugin URI: http://www.wpgothemes.com/
...
/*  Copyright 2009 David Gwyer (email : david(at)wpgothemes.com)
...
class WPGo_Admin_Log_Plugin {
	...
	public function render_form() {
		if ( ! current_user_can( 'manage_options' ) )
			wp_die( __( 'Sorry, you are not allowed to manage options for this plugin.' ) );
		?>
		<div class="wrap">
			...
			if( wp_get_referer() && isset( $_POST['wpgo_admin_log_clear'] ) ) {
				// Clear admin log
				...
				<table class="form-table">
					<tr>
						<td>
							...
							<div>Current log count: <?php echo substr_count( get_option( 'wpgo_admin_log' ), "\r\n" ); ?> (max. limit 200)<span style="font-style:italic;color:#666666;margin:0px 0px 30px 2px;"></span>
							</div>
						</td>
					</tr>
					<tr>
						<td Width="554">
							<div>
								<form action="<?php echo esc_url($this->currURL()); // current page url ?>" method="post" id="wpgo_admin_log_clear_form" style="display:inline;">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					<tr valign="top">
						<td>
							<label><input name="wpgo_admin_log_options[chk_ignore_adminlog]" type="checkbox" value="1" <?php if ( isset( $options['chk_ignore_adminlog'] ) ) {
									checked( '1', $options['chk_ignore_adminlog'] );
								} ?>> Ignore Admin Log Page?</label>
				...
				<tr>
					<td>
						<p style="margin-top:20px;font-style:italic;">
							<span><a href="http://www.facebook.com/wpgothemes" title="Our Facebook page" target="_blank"><img style="border:1px #ccc solid;" src="<?php echo plugins_url(); ?>/admin-log/images/facebook-icon.png"></a></span>
							...
							&nbsp;&nbsp;<span><a href="http://www.wpgothemes.com" title="wpgothemes.com" target="_blank"><img style="border:1px #ccc solid;" src="<?php echo plugins_url(); ?>/admin-log/images/pc-icon.png"></a></span>
						</p>
					</td>
					<td>
		...
		if ( is_admin() ) {
			...
			if ( $user ) {
				...
				if ( ! ( isset( $options['chk_ignore_adminlog'] ) && $options['chk_ignore_adminlog'] == "1" && ( $uri == "admin-log.php" || $uri == "options-general.php?page=admin-log%2Fadmin-log.php&settings-updated=true" ) ) ) {
					...
					$txt  = $date . ' ' . $user->display_name . '(' . $user->ID . ') => [' . $_SERVER['REMOTE_ADDR'] . '] ' . esc_url($url[1]) . "\r\n";
					$txt  = $log . $txt;
